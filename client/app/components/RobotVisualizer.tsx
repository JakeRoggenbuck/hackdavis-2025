...
const customCompletionSource = (context: CompletionContext) => {
  ...
  return {
    ...
    options: [
      ...
      ...registerNames
        .filter((reg) => reg.startsWith(word))
        .map((reg) => ({
          label: reg,
          type: "registerName",
        })),
      ...
      {isOpen && (
        <div className="mt-2 p-4 bg-[#1e1e1e] rounded-lg border border-[#333] text-sm text-gray-300">
          ...
          <ul className="space-y-2">
            <li>
              • Always start with a{" "}
              <code className="bg-[#2a2a2a] px-2 py-1 rounded">main:</code>{" "}
              label
            </li>
            <li>• Use labels to organize your code into reusable sections</li>
            <li>
              • Comments start with{" "}
              <code className="bg-[#2a2a2a] px-2 py-1 rounded">#</code>
            </li>
            <li>
              • Use <code className="bg-[#2a2a2a] px-2 py-1 rounded">jal</code>{" "}
              for function-like behavior
            </li>
            <li>
              • Be careful with movement values - too large values may cause the
              robot to move off-screen
            </li>
          </ul>
        </div>
      )}
...
main:
  ...
  const animate = (timestamp: number) => {
    ...
    const progress = Math.min(
      1,
      (timestamp - startTime.current) / ANIMATION_DURATION,
    );
    ...
    setRobotState((prev) => {
      ...
      const newRotation =
        prev.rotation + (prev.targetRotation - prev.rotation) * easeProgress;
  ...
  const executeLabel = async (
    label: Label,
    labels: Map<string, Label>,
    executedLabels: Set<string> = new Set(),
  ) => {
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      const arduinoResponse = await fetch(
        "http://localhost:8080/api/compile/arduino",
        {
          method: "POST",
          headers: {
      ...
      setCompilationStatus({
        status: "compiling",
        message: "Validating program structure...",
      });
      ...
      setCompilationStatus({
        status: "error",
        message:
          error instanceof Error ? error.message : "Error compiling code",
      });
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadStatus({
        status: "error",
        message:
          error instanceof Error ? error.message : "Error uploading to Arduino",
      });
    }
  };
